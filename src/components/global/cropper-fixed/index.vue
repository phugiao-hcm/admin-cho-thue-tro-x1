...
const crop = () => {
    try {
        ...
        imageCropper.value = canvas.toDataURL()
...
const onResize = (coordinates: any) => {
    setTimeout(async () => {
        await getScaleCanvas(coordinates)
        dialogVisibleCropper.value = false
        ui.isSubmitting = false
    }, 50)
}

const getScaleCanvas = (canvas: any) => {
    return new Promise<string>((resolve) => {
        const img = new Image()
        img.src = imageCropper.value || ''
        const scaleCanvas = document.createElement('canvas')
        const scale = Number(props.width) / canvas.width
